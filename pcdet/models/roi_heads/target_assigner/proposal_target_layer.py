...
class ProposalTargetLayer(nn.Module):
    ...
    def sample_rois_for_rcnn(self, batch_dict):
        ...
        for index in range(batch_size):
            ...
            if index in batch_dict['unlabeled_inds'] and not cfg['MODEL']['ROI_HEAD'].ENABLE_BASELINE:
                ...
                (sampled_inds, cur_reg_valid_mask, cur_cls_labels, roi_ious, 
                 gt_assignment, cur_interval_mask, center_distance, angle_diff, max_overlaps) = \
                    self.top_k_subsampler_unlabeled_rois(batch_dict, index)
            else:
                # Subsampler labeled/unlabeled ROIs using randomly balanced subsampler
                (sampled_inds, cur_reg_valid_mask, cur_cls_labels, roi_ious, 
                 gt_assignment, cur_interval_mask, center_distance, angle_diff, max_overlaps) = \
                    self.default_subsample_rois(batch_dict, index)
    ...
    '''
    def top_k_subsampler_unlabeled_rois(self, batch_dict, index):
    ...
    '''
    Subsample ROIs using Randomly balanced subsampler for labeled/unlabeled data.
    '''
    def default_subsample_rois(self, batch_dict, index):
        cur_roi = batch_dict['rois'][index]
        cur_gt_boxes = batch_dict['gt_boxes'][index]
        cur_roi_labels = batch_dict['roi_labels'][index]
